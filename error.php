...
error_reporting(0);
 $fechainicial = $_REQUEST['ERROR'];


$mysqli = sqlsrv_connect(Server() , connectionInfo());
$sql_statement = "SELECT * FROM proyecto.error WHERE fecha='".$fechainicial."'";
...
while ($row = sqlsrv_fetch_array($resultado,SQLSRV_FETCH_ASSOC)){ 
  $objPHPExcel->getActiveSheet()->setCellValue('A'.$fila ,strval($row['code'] ));
  $objPHPExcel->getActiveSheet()->setCellValue('B'.$fila ,strval($row['op'] ));
  $objPHPExcel->getActiveSheet()->setCellValue('C'.$fila ,strval($row['nombre_dispositivo'] ));
  $objPHPExcel->getActiveSheet()->setCellValue('D'.$fila ,strval($row['dirrecion_ip']) );
  $objPHPExcel->getActiveSheet()->setCellValue('E'.$fila ,strval($row['hora']) );
